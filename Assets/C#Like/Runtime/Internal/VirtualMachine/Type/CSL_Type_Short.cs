...
namespace CSharpLike
{
    namespace Internal
    {
        class CSL_Type_Short : RegHelper_Type
        {
            public CSL_Type_Short()
            ...
            {
                bool convertSuccess;
                object convertedObject = CSL_NumericTypeUtils.TryConvertTo<short>(src, targetType, out convertSuccess);
                if (convertSuccess)
                {
            ...
            public override object Math2Value(CSL_Content content, string code, object left, CSL_Content.Value right, out CSL_Type returntype)
            {
                ...
                object value = CSL_NumericTypeUtils.Math2Value<short>(code, left, right, out returntype, out math2ValueSuccess);
            ...
            public override bool MathLogic(CSL_Content content, TokenLogic code, object left, CSL_Content.Value right)
            {
                ...
                bool value = CSL_NumericTypeUtils.MathLogic<short>(code, left, right, out mathLogicSuccess);
                if (mathLogicSuccess)
                {
                    return value;
                }
